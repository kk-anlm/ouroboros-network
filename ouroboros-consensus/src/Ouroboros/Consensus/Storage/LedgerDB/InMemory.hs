...
ledgerDbWithAnchor runAlsoLegacy anchor = LedgerDB {
      ledgerDbCheckpoints =
        case runAlsoLegacy of
          RunBoth -> if isCandidateForUnstow anchor -- Function 'isCandidateForUnstow' is only defined if we 'RunBoth'.
                       then Just (Empty (Checkpoint (unstowLedgerTables anchor)))
                       else error "Requested to run with legacy DB but anchor loaded from disk has no in-mem UTxO"
          _       -> Nothing
