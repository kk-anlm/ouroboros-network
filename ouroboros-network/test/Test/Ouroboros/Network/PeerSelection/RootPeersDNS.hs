...
import           Control.Monad.Class.MonadSTM.Strict (MonadSTM, newTVarIO,
                     readTVar, traceTVarIO, toLazyTVar, MonadTraceSTM)
...
prop_local_singleSourceOfTruth mockRoots
                               dnsTimeoutScript
                               dnsLookupDelayScript =
    let tr = selectTraceEventsDynamic
     ...
     in fst
      $ foldl'
          (\(r, localRootSet) t ->
            case t of
              Solo curr ->
                let localRootSet' = Set.fromList
                                  . concatMap (Map.keys . snd)
                                  . toList
                                  $ curr
                    isMonotonic = localRootSet `Set.isSubsetOf` localRootSet'
                 in ( counterexample
                        (show localRootSet ++ " is not subset of "
                        ++ show localRootSet')
                        isMonotonic
                      .&&. r
                    , localRootSet')
          )
          (property True, Set.empty)
          tr
